...
export class AvlTree<K, T> {
  private _root: Node<K, T>;
  private _size: number;

  constructor(customCompare?: (a: K, b: K) => -1 | 0 | 1) {
    this._root = null;
    this._size = 0;
   ...
   */
  private _insert(key: K, value: T, root: Node<K, T>) {
   ...
   * @return {Node} The new tree root.
   */
  private _delete(key: K, root: Node<K, T>) {
   ...
   * @return {Object} The value of the node or null if it doesn't exist.
   */
  private _get(key: K, root: Node<K, T>): Node<K, T> {
    if (key === root.key) {
      return root;
    }
   ...
   */
  findMinimum() {
    return minValueNode<K, T>(this._root).key;
  }
   ...
   */
  findMaximum() {
    return maxValueNode<K, T>(this._root).key;
  }
 ...
 */
function maxValueNode<K, T>(root: Node<K, T>) {
  var current = root;
  while (current.right) {
 ...
 */
function minValueNode<K, T>(root: Node<K, T>) {
  var current = root;
  while (current.left) {
 ...
 */
function getBalanceState<K, T>(node: Node<K, T>) {
